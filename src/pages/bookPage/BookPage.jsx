...
export default function BookPage() {
  ...
  // Alterado: Se não tiver na estante, o status começa como null (nenhum selecionado)
  ...
  const handleFavorite = () => {
    ...
    // Se o livro ainda não estiver na estante ao favoritar, marcamos como "Quero Ler"
    if (!shelfItem) {
      const statusInicial = status || "queroLer";
      setStatus(statusInicial);
      addToShelf(book, statusInicial);
    }
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: isDark ? "#1A202C" : "#FAFAFA", fontFamily: "system-ui, sans-serif", color: isDark ? "#F7FAFC" : "#1A202C", display: "flex", flexDirection: "column" }}>
      ...
      <main style={{ width: "100%", maxWidth: "1050px", margin: "0 auto", padding: "24px", boxSizing: "border-box", display: "flex", gap: "40px", flex: 1 }}>
        ...
        <section style={{ display: "flex", flexDirection: "column", alignItems: "center", flexShrink: 0 }}>
          <div style={{ position: "relative", display: "inline-block" }}>
            <button
              style={{ position: "absolute", top: "12px", right: "12px", width: "42px", height: "42px", border: "none", borderRadius: "50%", background: "rgba(255, 255, 255, 0.9)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "18px", color: LARANJA, cursor: "pointer", boxShadow: "0 4px 10px rgba(0, 0, 0, 0.1)", zIndex: 2 }}
        ...
        <section style={{ flex: 1, display: "flex", flexDirection: "column", gap: "16px" }}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {STATUS_OPTIONS.map(([val, label]) => (
                <button key={val} onClick={() => handleStatusChange(val)}
                  style={{ padding: "8px 16px", borderRadius: "20px", border: status === val ? `2px solid ${LARANJA}` : `2px solid ${isDark ? "#4A5568" : "#E2E8F0"}`, cursor: "pointer", fontFamily: "inherit", fontSize: "12px", fontWeight: status === val ? "700" : "500", backgroundColor: status === val ? (isDark ? "#1A202C" : "#FFF") : (isDark ? "#1A202C" : "#F8FAFC"), color: status === val ? LARANJA : isDark ? "#A0AEC0" : "#4A5568", transition: "all 0.2s" }}>
                  {label}
                </button>
              ))}
            </div>
          ...
          <div style={cardStyle}>
            ...
            <div style={{ display: "flex", gap: "4px" }}>
              ...
              {rating > 0 && <span style={{ fontSize: "14px", color: isDark ? "#F7FAFC" : "#1A202C", alignSelf: "center", marginLeft: "10px", fontWeight: "bold" }}>{rating} de 5</span>}
            </div>
          </div>
          ...
          {showReview && (
            <div style={cardStyle}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "10px" }}>
                ...
                <button onClick={handleSave} style={{ padding: "6px 16px", color: "#FFFFFF", border: "none", borderRadius: "20px", cursor: "pointer", fontSize: "11px", fontWeight: "bold", backgroundColor: saved ? "#34A853" : LARANJA }}>
                  {saved ? "Salvo! ✓" : "Salvar"}
                </button>
              </div>
              <textarea style={{ width: "100%", minHeight: "80px", padding: "10px", backgroundColor: isDark ? "#1A202C" : "#F8FAFC", border: `1px solid ${isDark ? "#4A5568" : "#E2E8F0"}`, borderRadius: "8px", resize: "none", outline: "none", fontFamily: "inherit", fontSize: "13px", color: isDark ? "#F7FAFC" : "#1A202C", lineHeight: "1.5", boxSizing: "border-box" }} placeholder="O que você achou deste livro?" value={review} onChange={(e) => setReview(e.target.value)} />
            </div>
          )}
      ...
      <section style={{ width: "100%", maxWidth: "1050px", margin: "0 auto", padding: "16px 24px 64px", boxSizing: "border-box" }}>
        ...
        {book.publishedDate && <p style={{ fontSize: "12px", color: isDark ? "#A0AEC0" : "#718096", marginBottom: "16px", margin: 0 }}>{book.publishedDate} · {book.pageCount ? `${book.pageCount} páginas` : ""}</p>}
        <div style={{ fontSize: "15px", lineHeight: "1.7", color: isDark ? "#CBD5E0" : "#4A5568", marginTop: "16px" }} dangerouslySetInnerHTML={{ __html: book.description || "Sem descrição disponível." }} />
      </section>
    </div>
  );
}